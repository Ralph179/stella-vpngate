        ...
        while not self.stop_event.is_set():
            ...
            try:
                ...
                if now - last_check > int(os.getenv("CHECK_INTERVAL_SECONDS", str(CHECK_INTERVAL_SECONDS))) and load_nodes(self.data_dir):
                    ...
                    last_check = now
                if state.get("connection_enabled"):
                    if not state.get("active_openvpn_node_id"):
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        if route == "/":
            ...
            state["server_ip"] = public_server_ip()
    ...
    def do_POST(self) -> None:
        ...
        try:
            if route == "/api/login":
                ...
                nodes = check_nodes(DATA_DIR, openvpn=bool(body.get("openvpn", False)), full=bool(body.get("full", False)))
                state = load_state(DATA_DIR)
                self.send_json({
                    "ok": True,
                    "available": sum(1 for n in nodes if n.get("probe_status") == "available"),
                    "message": state.get("last_check_message", "检测完成"),
                })
...
<button onclick="tab('dash')">控制台</button><button onclick="tab('nodes')">节点</button><button onclick="tab('logs')">日志</button><a class="nav-link" href="https://t.me/esimuse" target="_blank" rel="noopener noreferrer">TG 群</a><button onclick="api('/api/logout',{{}}).then(()=>location='/{secret}')">退出</button>
</nav></header>
<main>
<div id="statusBar" class="status-bar">就绪</div>
<section id="dash"></section>
<section id="nodes" hidden><div class="toolbar"><select id="countryFilter" onchange="renderNodes()"></select><select id="statusFilter" onchange="renderNodes()"><option value="">全部状态</option><option value="available">可用</option><option value="unavailable">不可用</option><option value="not_checked">未检测</option><option value="blacklisted">已屏蔽</option></select><select id="ipTypeFilter" onchange="renderNodes()"><option value="">全部 IP 类型</option><option value="residential">住宅</option><option value="hosting">机房</option><option value="mobile">移动网络</option><option value="unknown">未知</option><option value="unchecked">未检测</option></select></div><div id="nodeTable"></div></section>
...
function setStatus(text, cls=''){statusBar.className='status-bar '+cls;statusBar.textContent=text}
async function runAction(label,path,body){setStatus(label+'中，请稍候…','busy');try{const j=await api(path,body||{});await refresh();setStatus((j.message||label+'完成'),'ok');return j}catch(e){setStatus(label+'失败','bad');throw e}}
...
function nodeLabel(n){if(!n||!n.id)return '未连接';const latency=n.latency_ms||n.ping||0;return `${n.country_short||'未知'} · ${n.ip||'-'} · ${latency} ms · ${ipTypeText(n.ip_type,n)}`}
...
async function refresh(){state=await get('/api/state');nodes=await get('/api/nodes');renderDash();renderNodes()}
async function toggleAutoSelect(){const enable=!state.connection_enabled;setStatus((enable?'开启':'关闭')+'自动选择节点中，请稍候…','busy');await api('/api/settings',{connection_enabled:enable});if(enable&&!state.active_openvpn_node_id)await api('/api/auto-connect',{});await refresh();setStatus('自动选择节点已'+(enable?'开启':'关闭'),'ok')}
function renderDash(){const active=nodes.find(n=>n.id===state.active_openvpn_node_id)||{};dash.innerHTML=`<div class="toolbar"><button onclick="runAction('检测代理出口','/api/check-proxy')">检测代理出口</button></div><div class="grid"><div class="metric">当前节点<b>${nodeLabel(active)}</b></div><div class="metric">自动选择<b>${state.connection_enabled?'已开启':'已关闭'}</b></div><div class="metric">本机 IP<b>${state.server_ip||'未检测'}</b></div><div class="metric">代理状态<b class="${state.proxy_ok?'ok':'bad'}">${state.proxy_ok?'正常':'异常'}</b></div><div class="metric">代理延迟<b>${state.proxy_latency_ms||0} ms</b></div></div><div class="panel"><b>本地代理</b><p>HTTP/SOCKS5：127.0.0.1:8888</p><p>本机 IP：${state.server_ip||'未检测'}</p><p class="hint">${state.proxy_error||''}</p></div>`}
function renderNodes(){let list=[...nodes];const c=countryFilter.value,s=statusFilter.value,t=ipTypeFilter.value;if(c)list=list.filter(n=>n.country_short===c);if(s)list=list.filter(n=>n.probe_status===s);if(t==='unchecked')list=list.filter(n=>!n.ip_type);else if(t)list=list.filter(n=>n.ip_type===t);list.sort((a,b)=>(a.latency_ms||a.ping||999999)-(b.latency_ms||b.ping||999999));const active=nodes.find(n=>n.id===state.active_openvpn_node_id)||{};countryFilter.innerHTML='<option value="">全部国家/地区</option>'+[...new Set(nodes.map(n=>n.country_short).filter(Boolean))].sort().map(x=>`<option ${x===c?'selected':''}>${x}</option>`).join('');nodeTable.innerHTML=`<div class="toolbar"><button onclick="runAction('更新节点','/api/fetch')">更新节点</button><button onclick="runAction('全量检测节点','/api/check',{full:true})">立即检测</button><button onclick="runAction('检测节点 IP 类型','/api/check-ip-types')">检测节点 IP 类型</button><button class="${state.connection_enabled?'':'secondary'}" onclick="toggleAutoSelect()">自动选择节点：${state.connection_enabled?'已开启':'已关闭'}</button><button onclick="runAction('断开连接','/api/disconnect')">断开连接</button></div><div class="grid"><div class="metric">当前节点<b>${nodeLabel(active)}</b></div><div class="metric">自动选择<b>${state.connection_enabled?'已开启':'已关闭'}</b></div><div class="metric">节点总数<b>${nodes.length}</b></div><div class="metric">住宅节点<b>${nodes.filter(n=>n.ip_type==='residential').length}</b></div><div class="metric">可用节点<b>${nodes.filter(n=>n.probe_status==='available').length}</b></div><div class="metric">已屏蔽节点<b>${nodes.filter(n=>n.probe_status==='blacklisted').length}</b></div></div><div class="panel"><p class="hint">${state.last_check_message||''}</p><p class="hint">${state.last_ip_type_check_message||''}</p></div><div class="table"><table><thead><tr><th>状态</th><th>国家/地区</th><th>IP</th><th>延迟</th><th>IP 类型</th><th>操作</th></tr></thead><tbody>${list.map(n=>{const blocked=n.probe_status==='blacklisted';return `<tr><td>${statusText(n.probe_status)}</td><td>${n.country_short}</td><td>${n.ip}</td><td>${n.latency_ms||n.ping||0} ms</td><td>${ipTypeText(n.ip_type,n)}</td><td>${blocked?'':`<button onclick="runAction('连接节点','/api/connect',{node_id:'${n.id}'})">连接</button>`} <button class="secondary" onclick="runAction('${blocked?'解除屏蔽':'屏蔽'}','/api/blacklist',{node_id:'${n.id}'})">${blocked?'解除屏蔽':'屏蔽'}</button></td></tr>`}).join('')}</tbody></table></div>`}
